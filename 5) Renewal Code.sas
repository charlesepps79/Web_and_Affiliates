...
%LET LOAN_ENTDATE_END1 = "2019-06-30"; /* CHANGE TO END OF THE MONTH */
...
DATA VW_LOANTABLE;
	SET DW.VW_LOAN(
		KEEP = SSNO1_RT7 BRACCTNO ID OWNBR SSNO1 SSNO2 LNAMT FINCHG
			   ...
			   TILA_LnAmt ORGST APRATE SRCD POCD POFFDATE PLDATE
...
PROC SQL;
	...
	SELECT * FROM ALL_APPS_3 WHERE ENTYRMONTH IN (201906); /* CHANGE */
QUIT;
...
PROC SQL;
	SELECT ENTYRMONTH, 
		   SUM(BOOKED) 
	FROM MONTH_BOOK 
	WHERE APPYRMONTH IN (201906) GROUP BY 1; /* CHANGE */
QUIT;
...
PROC SQL;
	...
	SELECT A.*, 
		   B.SSNO1,
		   B.TILA_LnAmt AS NETLOANAMOUNT_B, 
		   B.OWNBR, 
		   B.CLASSID, 
		   B.CLASSTRANSLATION, 
		   B.POCD, 
