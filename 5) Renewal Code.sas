...
%LET LOAN_ENTDATE_END1 = "2020-01-31"; /* CHANGE TO END OF THE MONTH */
...
PROC SQL;
	...
	SELECT * FROM ALL_LEADS_3 WHERE ENTYRMONTH IN (202001); /* CHANGE */
QUIT;

*** SUBSET FOR NON 'NB' AND 'FB' CUSTOMER TYPE ------------------- ***;
PROC SQL;
	CREATE TABLE ALL_LEAD4 AS
	SELECT ENTYRMONTH, 
		   BRACCTNO, 
		   NETLOANAMOUNT, 
...
PROC SQL;
	SELECT ENTYRMONTH, 
		   SUM(BOOKED) 
	FROM MONTH_BOOK 
	WHERE LEADYRMONTH IN (202001) GROUP BY 1; /* CHANGE */
QUIT;

*** JOIN WITH LOAN TABLE ----------------------------------------- ***;
PROC SQL;
	CREATE TABLE ALL_LEAD5 AS 
	SELECT A.*, 
		   B.SSNO1,
...
PROC SQL;
	...
	SELECT *,
		   CASE WHEN LENGTH(OWNBR) = 1 THEN '000' || OWNBR
				...
				WHEN LENGTH(OWNBR) > 4 THEN OWNBR 
					ELSE OWNBR 
			END AS NEW_OWNBR FROM ALL_LEAD5; 
QUIT;

PROC SQL;
	CREATE TABLE ALL_LEAD7 AS
	SELECT *, 
...
PROC EXPORT 
	DATA = ALL_LEAD9 
	OUTFILE = "&MAIN_DIR\ALL_LEAD9.xlsx" 
	DBMS = EXCEL 
	REPLACE;
RUN;
...
PROC SQL;
	...
	SELECT A.*, 
		   ...
		   B.RENEW_AMT
	FROM ALL_LEADS_3 A 
	LEFT JOIN ALL_LEAD9 B ON A.BRACCTNO = B.RENEW_BRACCTNO; 
QUIT;
