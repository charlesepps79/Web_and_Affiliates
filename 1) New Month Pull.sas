...
%LET MAIN_DIR = 
	\\mktg-app01\E\cepps\Web_Report\Reports\02_2019;
*** CHANGE ONLY WHEN ROSTER FILE CHANGES ------------------------- ***;
%LET ROSTER_LOC = 
"\\mktg-app01\E\cepps\Web_Report\BranchRosterFile\Branch Roster 2-21-19.xlsx";

*** CHANGE MONTH_NAME TO APP MONTH ------------------------------- ***;
%LET NEW_MONTH_FILE = FEB_APPS;
*** CHANGE DATE TO START OF APP MONTH ---------------------------- ***;
%LET LOAN_ENT_DATE_BEGIN = "2019-02-01";
*** CHANGE DATE TO END OF APP MONTH ------------------------------ ***;
%LET LOAN_ENT_DATE_END = "2019-02-28";


*** NO CHANGE - ENSURE CURRENT MONTH APP FILE -------------------- ***;
%LET AIP_REPORT_LOC = 
"\\mktg-app01\E\cepps\Web_Report\Reports\02_2019\Application Internet report 2019-02-01 to 2019-02-28.xlsx";
%LET TAB_NAME = Application Internet Report;

*** CHANGE TO LAST MONTH ALL APPS FILENAME ----------------------- ***;
%LET ALL_APPS_HIST_LOC = "&MAIN_DIR\ALL_APPS_JAN2019.xlsx";

*** THIS IS FILE NAME OF CURRENT ALL APPS FILE ------------------- ***;
%LET ALL_APPS_2_EXPORT = ALL_APPS_FEB_2019;
%LET VW_L_EXPORT = VW_L_FEB_2019; /* THIS IS FILENAME OF VW_L FILE */

%LET APP_IMPORT_FILE = APP_IMPORT_FILE_2; 
*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ALL_APPS_FILE = ALL_APPS_JAN_FINAL;
*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ALL_APPS_FILE_2 = ALL_APPS_JAN_FINAL;

%LET RECENT_MONTH_NO = 201902; /* YYYYMM CHANGE TO APP MONTH */
%LET ONE_MO_AGO = 201901; /* YYYYMM CHANGE TO APP MONTH - 1 */
%LET TWO_MO_AGO = 201812;	/* YYYYMM CHANGE TO APP MONTH - 2 */

*** CHANGE MONTH NAME TO APP MONTH - 2 MONTH --------------------- ***;
%LET TWO_MO_BOOKED = DEC_APPS_B;
%LET TWO_MO_UNBOOKED = DEC_APPS_UNB;
%LET APPS_MINUS_2_MO_AGO = APPS_EXCEPT_DEC;

*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ONE_MO_BOOKED = JAN_APPS_B;
%LET ONE_MO_UNBOOKED = JAN_APPS_UNB;
%LET APPS_MINUS_1_MO_AGO = APPS_EXCEPT_JAN;
