...
%LET MAIN_DIR = 
	\\mktg-app01\E\cepps\Web_Report\Reports\09_2019;
...
%LET NEW_MONTH_FILE = SEP_APPS;
*** CHANGE DATE TO START OF APP MONTH ---------------------------- ***;
%LET LOAN_ENT_DATE_BEGIN = "2019-09-01";
*** CHANGE DATE TO END OF APP MONTH ------------------------------ ***;
%LET LOAN_ENT_DATE_END = "2019-09-30";


*** NO CHANGE - ENSURE CURRENT MONTH APP FILE -------------------- ***;
%LET AIP_REPORT_LOC = 
"\\mktg-app01\E\cepps\Web_Report\Reports\09_2019\Application Internet Report 9-1 9-30-2019.xlsx";
%LET TAB_NAME = Application Internet Report;

*** CHANGE TO LAST MONTH ALL APPS FILENAME ----------------------- ***;
%LET ALL_APPS_HIST_LOC = "&MAIN_DIR\ALL_APPS_AUG2019.xlsx";

*** THIS IS FILE NAME OF CURRENT ALL APPS FILE ------------------- ***;
%LET ALL_APPS_2_EXPORT = ALL_APPS_SEP_2019;
%LET VW_L_EXPORT = VW_L_JUN_2019; /* THIS IS FILENAME OF VW_L FILE */

%LET APP_IMPORT_FILE = APP_IMPORT_FILE_2; 
*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ALL_APPS_FILE = ALL_APPS_AUG_FINAL;
*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ALL_APPS_FILE_2 = ALL_APPS_AUG_FINAL;

%LET RECENT_MONTH_NO = 201909; /* YYYYMM CHANGE TO APP MONTH */
%LET ONE_MO_AGO = 201908; /* YYYYMM CHANGE TO APP MONTH - 1 */
%LET TWO_MO_AGO = 201907;	/* YYYYMM CHANGE TO APP MONTH - 2 */

*** CHANGE MONTH NAME TO APP MONTH - 2 MONTH --------------------- ***;
%LET TWO_MO_BOOKED = JUL_APPS_B;
%LET TWO_MO_UNBOOKED = JUL_APPS_UNB;
%LET APPS_MINUS_2_MO_AGO = APPS_EXCEPT_JUL;

*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ONE_MO_BOOKED = AUG_APPS_B;
%LET ONE_MO_UNBOOKED = AUG_APPS_UNB;
%LET APPS_MINUS_1_MO_AGO = APPS_EXCEPT_AUG;
...
DATA VP_LIST;
	SET VP_LIST;
	RENAME 'Vice President'n = VP ;
	OWNBR = put('BRNUM'n,z4.);
 	format a z4.;
	KEEP  SUPERVISOR OWNBR 'BRNUM'n 'Vice President'n ZIP;
RUN;
