...
%LET MAIN_DIR = 
	\\mktg-app01\E\cepps\Web_Report\Reports\10_2018;
*** CHANGE ONLY WHEN ROSTER FILE CHANGES ------------------------- ***;
%LET ROSTER_LOC = 
"\\mktg-app01\E\cepps\Web_Report\BranchRosterFile\Rosters 10-30-18.xlsx";

*** CHANGE MONTH_NAME TO APP MONTH ------------------------------- ***;
%LET NEW_MONTH_FILE = OCT_APPS;
*** CHANGE DATE TO START OF APP MONTH ---------------------------- ***;
%LET LOAN_ENT_DATE_BEGIN = "2018-10-01";
*** CHANGE DATE TO END OF APP MONTH ------------------------------ ***;
%LET LOAN_ENT_DATE_END = "2018-10-31";


*** NO CHANGE - ENSURE CURRENT MONTH APP FILE -------------------- ***;
%LET AIP_REPORT_LOC = 
"\\mktg-app01\E\cepps\Web_Report\Reports\10_2018\Application Internet Report 2018-10-01 2018-10-31.xlsx";
%LET TAB_NAME = Application Internet Report;

*** CHANGE TO LAST MONTH ALL APPS FILENAME ----------------------- ***;
%LET ALL_APPS_HIST_LOC = "&MAIN_DIR\ALL_APPS_SEP2018.xlsx";

*** THIS IS FILE NAME OF CURRENT ALL APPS FILE ------------------- ***;
%LET ALL_APPS_2_EXPORT = ALL_APPS_OCT_2018;
%LET VW_L_EXPORT = VW_L_OCT_2018; /* THIS IS FILENAME OF VW_L FILE */

%LET APP_IMPORT_FILE = APP_IMPORT_FILE_2; 
*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ALL_APPS_FILE = ALL_APPS_SEP_FINAL;
*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ALL_APPS_FILE_2 = ALL_APPS_SEP_FINAL;

%LET RECENT_MONTH_NO = 201810; /* YYYYMM CHANGE TO APP MONTH */
%LET ONE_MO_AGO = 201809; /* YYYYMM CHANGE TO APP MONTH - 1 */
%LET TWO_MO_AGO = 201808;	/* YYYYMM CHANGE TO APP MONTH - 2 */

*** CHANGE MONTH NAME TO APP MONTH - 2 MONTH --------------------- ***;
%LET TWO_MO_BOOKED = AUG_APPS_B;
%LET TWO_MO_UNBOOKED = AUG_APPS_UNB;
%LET APPS_MINUS_2_MO_AGO = APPS_EXCEPT_AUG;

*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ONE_MO_BOOKED = SEP_APPS_B;
%LET ONE_MO_UNBOOKED = SEP_APPS_UNB;
%LET APPS_MINUS_1_MO_AGO = APPS_EXCEPT_SEP;
