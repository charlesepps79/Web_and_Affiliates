...
%LET MAIN_DIR = 
	\\mktg-app01\E\cepps\Web_Report\Reports\06_2019;
*** CHANGE ONLY WHEN ROSTER FILE CHANGES ------------------------- ***;
%LET ROSTER_LOC = 
"\\mktg-app01\E\cepps\Web_Report\BranchRosterFile\Branch Roster 05-01-19.xlsx";

*** CHANGE MONTH_NAME TO APP MONTH ------------------------------- ***;
%LET NEW_MONTH_FILE = JUN_APPS;
*** CHANGE DATE TO START OF APP MONTH ---------------------------- ***;
%LET LOAN_ENT_DATE_BEGIN = "2019-06-01";
*** CHANGE DATE TO END OF APP MONTH ------------------------------ ***;
%LET LOAN_ENT_DATE_END = "2019-06-30";


*** NO CHANGE - ENSURE CURRENT MONTH APP FILE -------------------- ***;
%LET AIP_REPORT_LOC = 
"\\mktg-app01\E\cepps\Web_Report\Reports\06_2019\Application Internet report june 2019.xlsx";
%LET TAB_NAME = Application Internet Report;

*** CHANGE TO LAST MONTH ALL APPS FILENAME ----------------------- ***;
%LET ALL_APPS_HIST_LOC = "&MAIN_DIR\ALL_APPS_MAY2019.xlsx";

*** THIS IS FILE NAME OF CURRENT ALL APPS FILE ------------------- ***;
%LET ALL_APPS_2_EXPORT = ALL_APPS_JUN_2019;
...
%LET ALL_APPS_FILE = ALL_APPS_MAY_FINAL;
*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ALL_APPS_FILE_2 = ALL_APPS_MAY_FINAL;

%LET RECENT_MONTH_NO = 201906; /* YYYYMM CHANGE TO APP MONTH */
%LET ONE_MO_AGO = 201905; /* YYYYMM CHANGE TO APP MONTH - 1 */
%LET TWO_MO_AGO = 201904;	/* YYYYMM CHANGE TO APP MONTH - 2 */
...
%LET ONE_MO_UNBOOKED = MAY_APPS_UNB;
%LET APPS_MINUS_1_MO_AGO = APPS_EXCEPT_MAY;
...
DATA FINAL;
	...
	*** costperloan = 200 ---------------------------------------- ***;
	...
	IF BOOKED = 1 AND AFFILIATE = 'Credit Karma' THEN DO;
		IF TILA_LnAmt > 2500 THEN COSTPERLOAN = 200;
