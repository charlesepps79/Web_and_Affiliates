...
%LET MAIN_DIR = 
	\\mktg-app01\E\cepps\Web_Report\Reports\08_2018;
*** CHANGE ONLY WHEN ROSTER FILE CHANGES ------------------------- ***;
%LET ROSTER_LOC = 
"\\mktg-app01\E\cepps\Web_Report\BranchRosterFile\Rosters 8-28-18.xlsx";

*** CHANGE MONTH_NAME TO APP MONTH ------------------------------- ***;
%LET NEW_MONTH_FILE = AUG_APPS;
*** CHANGE DATE TO START OF APP MONTH ---------------------------- ***;
%LET LOAN_ENT_DATE_BEGIN = "2018-08-01";
*** CHANGE DATE TO END OF APP MONTH ------------------------------ ***;
%LET LOAN_ENT_DATE_END = "2018-08-31";


*** NO CHANGE - ENSURE CURRENT MONTH APP FILE -------------------- ***;
%LET AIP_REPORT_LOC = 
"\\mktg-app01\E\cepps\Web_Report\Reports\08_2018\Application Internet Report 2018-08-01 to 2018-08-31.xlsx";
%LET TAB_NAME = Application Internet Report;

*** CHANGE TO LAST MONTH ALL APPS FILENAME ----------------------- ***;
%LET ALL_APPS_HIST_LOC = "&MAIN_DIR\ALL_APPS_JUL2018.xlsx";

*** THIS IS FILE NAME OF CURRENT ALL APPS FILE ------------------- ***;
%LET ALL_APPS_2_EXPORT = ALL_APPS_AUG_2018;
%LET VW_L_EXPORT = VW_L_AUG_2018; /* THIS IS FILENAME OF VW_L FILE */

%LET APP_IMPORT_FILE = APP_IMPORT_FILE_2; 
*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ALL_APPS_FILE = ALL_APPS_JUL_FINAL;
*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ALL_APPS_FILE_2 = ALL_APPS_JUL_FINAL;

%LET RECENT_MONTH_NO = 201808; /* YYYYMM CHANGE TO APP MONTH */
%LET ONE_MO_AGO = 201807; /* YYYYMM CHANGE TO APP MONTH - 1 */
%LET TWO_MO_AGO = 201806;	/* YYYYMM CHANGE TO APP MONTH - 2 */

*** CHANGE MONTH NAME TO APP MONTH - 2 MONTH --------------------- ***;
%LET TWO_MO_BOOKED = JUN_APPS_B;
%LET TWO_MO_UNBOOKED = JUN_APPS_UNB;
%LET APPS_MINUS_2_MO_AGO = APPS_EXCEPT_JUN;

*** CHANGE MONTH NAME TO APP MONTH - 1 MONTH --------------------- ***;
%LET ONE_MO_BOOKED = JUL_APPS_B;
%LET ONE_MO_UNBOOKED = JUL_APPS_UNB;
%LET APPS_MINUS_1_MO_AGO = APPS_EXCEPT_JUL;
